...
module.exports.CreateWebServer = function (args) {
    ...
    obj.app.ws('/webrelay.ashx', function (ws, req) {
        ...
        ws.forwardclient.on('data', function (data) {
            ...
            try { ws.send(Buffer.from(data, "ascii")); } catch (e) { }
        });
    ...
    if (args.any != null) {
        obj.app.listen(port, function () { console.log('Hamonize-AMT Manager running on http://*:' + port + '.'); });
    } else {
        obj.app.listen(port, '127.0.0.1', function () { console.log('Hamonize-AMT Manager running on http://127.0.0.1:' + port + '.'); });
    }
